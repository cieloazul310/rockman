...
export function getYomi(artistName: string, kana?: string | null): string {
  ...
  if (the === "The " || the === "THE " || the === "the ")
    return kanaToHira(artistName.slice(4)).toLowerCase();
  return kanaToHira(kana || artistName).toLowerCase();
}

export function sortByYomi(
  a: Pick<ArtistListItem, "name" | "kana">,
  b: Pick<ArtistListItem, "name" | "kana">,
): number {
  return getYomi(a.name, a.kana ?? undefined).localeCompare(
    getYomi(b.name, b.kana ?? undefined),
  );
}
...
export function sortByPrograms(
  ...
  return (
    -(a.program.programsCount - b.program.programsCount) || sortByYomi(a, b)
  );
}
