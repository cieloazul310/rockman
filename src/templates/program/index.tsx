...
type ProgramTemplateData = {
  ...
  previous:
    | (Pick<
        Program,
        "slug" | "week" | "date" | "title" | "subtitle" | "image" | "categories"
      > & {
...
function ProgramTemplate({
  data,
}: PageProps<ProgramTemplateData, ProgramTemplateContext>) {
      ...
      left={
        previous
          ? {
              ...
              secondaryText: `第${previous.week}回`,
            }
          : undefined
      }
      right={
  ...
  const tabs = [
    previous ? (
      <ProgramTonarinoTab key={previous.title} item={previous} />
    ) : null,
...
export function Head({
  data,
}: HeadProps<ProgramTemplateData, ProgramTemplateContext>) {
  const { program } = data;
