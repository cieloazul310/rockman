...
function ProgramPageHeader({
  ...
  return (
    <Jumbotron component="header" maxWidth="md">
      <Box display="flex" flexGrow={1} width={1} height={1} flexDirection="column" justifyContent="center">
        <Stack spacing={2} alignItems={{ xs: 'center', md: 'start' }}>
          ...
          <Stack direction="row" spacing={1}>
            {program.categories.map((category) => (
              <AppLinkButton href="/categories/" state={{ category }} color="inherit" variant="outlined" size="small" key={category}>
                {category}
              </AppLinkButton>
            ))}
          </Stack>
