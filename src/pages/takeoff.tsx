...
function TakeOff({ data }: PageProps<TakeOffQueryData>) {
  ...
  const items: { title: string; oaLength?: number; tunesLength?: number }[] =
    React.useMemo(() => {
  ...
  return (
    <TabPageTemplate
      ...
      getCounterText={({ oaLength, tunesLength }) =>
        oaLength && tunesLength ? `${oaLength}/${tunesLength}曲` : undefined
      }
    >
      {[
        ...albums.nodes.map((node) => (
          <SectionWrapper component="article" key={node.id}>
            <Jumbotron
              ...
              footerText={`${
                node.tunes.filter((tune) => tune.program.length).length
              } / ${node.tunes.length}曲`}
            />
...
export const query = graphql`
  query TakeOff {
    albums: allSpitzAlbum(
      filter: { albumIdNum: { lte: 100 } }
      sort: { albumIdNum: ASC }
    ) {
      nodes {
        ...albumItem
    ...
    others: allSpitzAlbum(
      filter: { albumIdNum: { gte: 100 } }
      sort: { albumIdNum: ASC }
    ) {
      nodes {
        ...albumItem
