 ...
 * 3.
 */
export default async function onCreateResolvers({
  createResolvers,
}: CreateResolversArgs) {
  const resolvers = {
    Query: {
      allTunes: {
        ...
        resolve: async (
          source: unknown,
          { artist, ...otherArgs }: AllTunesQueryArgs,
        ...
        resolve: async (
          ...
          const { entries } = await context.nodeModel.findAll<Program<"node">>({
            ...
            query: {
              filter: {
                playlist: {
                  elemMatch: {
                    selector: { regex: "/^(?!.*草野マサムネ).*$/" },
                  },
                },
              },
            },
          });
          const allPrograms = Array.from(entries).sort(
            (a, b) => a.week - b.week,
          );
          const allSelectorNames = new Set(
            allPrograms.reduce<string[]>(
              (accum, curr) => [
                ...accum,
                ...curr.playlist.map(({ selector }) => selector),
              ],
              [],
            ),
          );
          ...
          const allSelectors = Array.from(allSelectorNames, (name) => {
            const programs = allPrograms
              .filter(({ playlist }) =>
                playlist.map(({ selector }) => selector).includes(name),
              )
            ...
            return {
              ...
              tunesCount: programs.reduce(
                (accum, curr) => accum + curr.playlist.length,
                0,
              ),
            };
          });
